...
namespace Equips
{
    public partial class frmMain : Form
    {
        ...
        private bool CSVFormatShown { get; set; }
        ...
        private void btnCreateTeam_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (lblActive is not null && lblActive.IsCompleted)
                        lblActive.Dispose();
                    lblActive = ShowAndHideInformationLabel(2, $@"Nou equip amb l'abreviació {txtABV.Text} creat.");
                    btnLoadAllTeams_Click(sender, e);
                }
            }
        }
        ...
        private void mnuOpenExplorerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "\nABV;Name;Butget;LogoLink\nIf the budget is null, a random value will be generated",
                ...
                using (StreamReader sr = new StreamReader(new FileStream(path, FileMode.Open), new UTF8Encoding())) // do anything you want, e.g. read it
                {
                    ...
                    try
                    {
                        while (line is not null)
                        {
                            ...
                            Team t;
                            t = Int32.TryParse(sp[2], out var value) ?
                                new Team(sp[1], sp[0], value, sp[3]) :
                                new Team(sp[1], sp[0], new Random().Next(0, 1500), sp[3]);
                            dao.Save(t);
                            line = sr.ReadLine();
                        }
                    }
        ...
        private void totalEquipsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            txtTotalEquips.Text = dao.Count().ToString();
        }

        private void promigDelsPressupostosToolStripMenuItem_Click(object sender, EventArgs e)
        {
            txtPromigPresup.Text = dao.Promig().ToString();
        }
    }
}
